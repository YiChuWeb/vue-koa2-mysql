...
trouter.get("/findAdminByUsername", async function (ctx, next){
    let _this = ctx;
    let reqData = _this.request.query;
    let admins = await handler.findAdminByParams(reqData);
    let _body = {
        status: 1,
        admins: admins
    }
...
trouter.delete("/deleteOrgById",async function (ctx, next){
    ...
    let adminCount = await handler.findAdminCountByParams({org:orgId});
    let body = {};
    if(adminCount){
        body = {status:0,errorMessage:"删除失败，该组织机构上已有用户"}
    }else{
        let result = await handler.deleteOrgById(orgId);
        body = result.result.ok ? { status: 1} : {status:0,errorMessage:"删除失败，请重试"};
    }
    ctx.body = body;
});
...
trouter.delete("/deleteRole",async function (ctx, next){
    ...
    let adminCount = await handler.findAdminCountByParams({adminRoles:roleId});
    let body = {};
    if(adminCount){
        body = {status:0,errorMessage:"删除失败，该角色上已有用户"}
    }else{
        let result = await handler.deleteRoleById(roleId);
        body = result.result.ok ? { status: 1} : {status:0,errorMessage:"删除失败，请重试"};
    }
    ctx.body = body;
});
