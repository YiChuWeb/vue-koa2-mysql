...
class AdminHandler{
    ...
    findAdminByParams(conditions){
        return adminModel.find(conditions).exec();
    }

    findAdminCountByParams(conditions){
        return adminModel.find(conditions).count().exec();
    }
    ...
    findPermission(id){
        let _this = this;
        return _this.findAdminByParams({_id:id}).then(function(adminRes){
            return _this.findResourcesByRoleId(adminRes[0].adminRoles)
        }).then(function(res){
            let adminRoles = res;
            let resourceArr = [];
